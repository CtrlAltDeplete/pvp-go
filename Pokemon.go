...
func (pokemon *Pokemon) MaximizeStats(baseStats map[string]float64) map[string]float64 {
	...
	for _, cpDto := range CP_DAO.FindAll() {
...
func NewPokemon(pokemonDto PokemonDto, fastMoveDto MoveDto, chargeMoveDtos []MoveDto) *Pokemon {
	var (
		p                  = Pokemon{}
		typesDto           *TypeDto
		firstTypeName      string
		secondTypeName     string
		typeMultiplierDtos []TypeMultiplierDto
		err                error
	)
	...
	p.typeIds = []int64{}
	err, typesDto = TYPES_DAO.FindSingleWhere("id = ?", pokemonDto.TypeId())
	CheckError(err)
	if typesDto.SecondTypeNullable().Valid {
		firstTypeName = typesDto.FirstType()
		err, firstTypeDto := TYPES_DAO.FindSingleByType(firstTypeName)
		CheckError(err)
		p.typeIds = append(p.typeIds, firstTypeDto.Id())

		secondTypeName = typesDto.SecondType()
		err, secondTypeDto := TYPES_DAO.FindSingleByType(secondTypeName)
		CheckError(err)
	...
	typeMultiplierDtos = TYPE_MULTIPLIER_DAO.FindAllByReceivingType(typesDto.Id())
	...
	if pokemonDto.OptLevel() <= 1 {
		...
		err, cpmDto := CP_DAO.FindByLevel(p.level)
		CheckError(err)
